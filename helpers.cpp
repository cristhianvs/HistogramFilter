...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	vector<float> row_line;
	for (int i=0;i<row_grid;i++){
		row_line.clear();
		for (int j=0;j<col_grid;j++){
			row_line.push_back(grid[i][j]/total);
		}
		newGrid.push_back(row_line);
	}
	return newGrid;
}
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
    ...
    vector < vector <float> > window_initial(3,vector <float> (3, 0.0));
    window_initial[0][0]=corner_prob;
    window_initial[0][1]=adjacent_prob;
    window_initial[0][2]=corner_prob;
    window_initial[1][0]=adjacent_prob;
    window_initial[1][1]=center_prob;
    window_initial[1][2]=adjacent_prob;
    window_initial[2][0]=corner_prob;
    window_initial[2][1]=adjacent_prob;
    window_initial[2][2]=corner_prob;

    newGrid.assign(3,vector <float> (3, 0.0));
    float grid_val;
    float mult;
    int new_i;
    int new_j;
...
/**
	Mod operator in C++ equivalent to Python mod operator 
*/
